...
<body>
	...
	<div class="container" style="margin-top: 20px;">
		<div class="alert alert-success alert-dismissible fade show text-center"
			...
			<i class="fas fa-check-circle"></i><strong> Success!!</strong> ${param.message }
			<button type="button" class="btn-close" data-bs-dismiss="alert"
				aria-label="Close"></button>
		</div>
	</div>
	</c:if>
	<c:if test="${param.isSuccess eq false}">
	<div class="container" style="margin-top: 20px;">
		<div class="alert alert-warning alert-dismissible fade show text-center"
			role="alert" style="border: 1px solid;">
			<i class="fas fa-exclamation-triangle"></i><strong> Failure!!</strong> ${param.message }
			<button type="button" class="btn-close" data-bs-dismiss="alert"
				aria-label="Close"></button>
		</div>
	</div>
	...
	<c:if test="${not empty listOfCustomers}">
		...
		<div class="container">
			...
			<table class="table table-hover table-bordered text-center" style="background-color: #D6EEEE;">
				...
				<tbody>
					<c:forEach var="listValue" items="${listOfCustomers}">
						<tr>
